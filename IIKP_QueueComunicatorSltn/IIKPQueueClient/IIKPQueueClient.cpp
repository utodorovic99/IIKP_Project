        ...
        {  
            ...
            #pragma omp atomic capture 
            {
                ...
                while (cycles>0 && !*(params.end_thr_flag))
                {
                    ...
                    params.in_buffer->Dequeue(accept_messg_buff);
    ...
    DWORD WINAPI ClientRegisterThr(LPVOID lp_param)
    {
        ...
        do
        {
            ...
            else if (i_result > 0)
            {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            if (send(*new_sock_params->socket, services_conn_params, strlen(services_conn_params), 0) != SOCKET_ERROR)                                                 // Send services offer                                
                            {
                                ...
                                waiting_fetch_resp->Insert(new_sock_params, &waiting_fetch_resp);                                                                                     // Wait for Connect message
                            }
                            else
                            {
                                EnterCriticalSection(&disp);
                ...
                for (SOCKET_DATA* pending_it = waiting_fetch_resp; pending_it != NULL; pending_it = pending_it->next)                                   // Check if connect is sent
                {
                        ...
                        if (bytes_rec > 0)
                        {
                            ...
                            if (params.service_names->Contains(chk_name->name, params.service_names) && !services_taken->Contains(chk_name->name, services_taken))  // Check if exists and is free
                            {
                                ...
                                if (send(*(pending_it->socket), "ACK\0", strlen("ACK\0"), 0) != SOCKET_ERROR)                                           // Confirm connect to client
                                {
                                    ...
                                    params.accept_triggers[target_service];                                                                             // Invoke thread start working on fetched socket
                                }
                    ...
                    if (FD_ISSET(*(pending_it->socket), &exc_set))
                    {
                        EnterCriticalSection(&disp);
    ...
    DWORD WINAPI ClientReqHandleThr(LPVOID lp_param)
    {
        ...
        do
        {
            ...
            #pragma omp atomic capture  // Send batch in atomic region
            {
                ...
                while (params.out_buffer->Dequeue(message_buff))                                                                              // Send buffered messages when client connected
                {
                    ...
                    if (FD_ISSET(*params.socket_data_accepted->socket, &read_set))   // Client sent something
                    {
                        ...
                        if (bytes_rec > 0)
                        {
                            ...
                            case CLIENT_ENQ:
                            {
                                ...
                                memset(message_buff, 0, MAX_MESSAGE_SIZE);
                                memcpy(message_buff, (char*)&code, 2);
                                memcpy(message_buff+2, params.service_name, strlen(params.service_name));
                                memcpy(message_buff + 2+ strlen(params.service_name)+1, recv_msg->content, strlen(recv_msg->content));
                                memcpy(message_buff + 2 + strlen(params.service_name) + 1+ strlen(recv_msg->content)+1, &msg_id_cnt, 4);
                                ++msg_id_cnt;
            ...
            #pragma omp atomic capture  // Send what is buffered
                {
                    memset(message_buff, 0, MAX_MESSAGE_SIZE);
                    EnterCriticalSection(&disp);    // Single lock because inside atomic region
                    while (params.out_buffer->Dequeue(message_buff + 2))
                    {
                        //Send
                                        ...
                                        while (new_names_it != NULL)
                                        {
                                            ...
                                            case(true):
                                            {
                                                ...
                                                *GC_handle->handle = CreateThread(NULL, 0, &ServiceGCThr, GC_thr_param, 0, params.threadIDs + thread_counter);
                                                ...
                                                client_thr_param->accept_trigger = &(params.accept_triggers[(*params.service_names)->Count(*params.service_names)]);    // Index of service wich would later be inserted at MARKER: #1
